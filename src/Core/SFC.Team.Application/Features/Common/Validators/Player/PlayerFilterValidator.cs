...
namespace SFC.Team.Application.Features.Common.Validators.Player;
        ...
        When(p => p?.Profile?.General != null, () =>
        {
            ...
            When(p => p.Profile.General.Tags?.Any() ?? false, () =>
            {
                RuleFor(p => p.Profile.General.Tags)
                    ...
                    .WithMessage(Localization.InvalidTagsSize.BuildValidationMessage(nameof(PlayerGeneralProfileFilterDto.Tags), ValidationConstants.TagsMaxLength));
            ...
            When(p => p.Profile.General.Availability?.Days?.Any() ?? false, () =>
            {
                RuleFor(p => p.Profile.General.Availability.Days)
                    ...
                    .WithMessage(Localization.AvailableDaysSize.BuildValidationMessage(nameof(PlayerAvailabilityLimitFilterDto.Days), Enum.GetNames(typeof(DayOfWeek)).Length));
